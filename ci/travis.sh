...
if [ "$USE_PYPY_NIGHTLY" = "1" ]; then
    curl -fLo pypy.tar.bz2 http://buildbot.pypy.org/nightly/py3.6/pypy-c-jit-latest-linux64.tar.bz2
...
if [ "$CHECK_DOCS" = "1" ]; then
    ...
    pytest -W error -ra -v --pyargs tricycle --cov=tricycle --cov-config=../.coveragerc --verbose
